...
int BfmWrite( con_Handle_t handle
             ...
             , unsigned int  length) // a unit of word (4-byte)
{
#ifdef RIGOR
   if (data==NULL) {
...
#endif
#if 1
 // in order to speed-up at USB-side
   unsigned int num;
   static unsigned int tbuf[1024+4];
   tbuf[0] = (2<<16) // command+address
           | ((0b0010&0xF)<<12) // control packet
           | ((0x0&0xF)<<4); // transactor
   GET_CMD(tbuf[1], 0, 1, 0, 0, size>>1, 1, 0, 0, 1, length-1);
           //       EI,WR,LK,EX, SZ     ,BT,PR,CA,ID,   BL
   tbuf[2] = addr;
   // to push BFM data for write
   // - control-flit for data
   tbuf[3] = (length<<16) //  command+data
           | ((0b0100&0xF)<<12) // control packet
           | ((0x0&0xF)<<4); // transactor
   for (int idx=0; (idx<1024)&&(idx<length); idx++) {
        tbuf[idx+4] = data[idx];
   }
   unsigned int done=0;
   unsigned int *pbuf=tbuf;
   for (num=(length<=1024) ? length+4 : 1024+4, done=0; num>0; num -= done, pbuf += done) {
   ...
   if (done<length) {
       num = length - done;
       for (done=0; num>0; num -= done, pbuf += done) {
            unsigned int zlp = ((num*4)%handle->usb.bulk_max_pkt_size_out) ? 0 : 1;
            if (conStreamWrite(handle, (void *)pbuf, num, &done, zlp)) {
                printf("%s() something went wrong: %d\n", __FUNCTION__, done);
                return -6;
...
//------------------------------------------------------------------------------
// Active-high IRQ and FIQ.
//
// Return <0 on failure, 0 on success.
CONFMC_BFM_API
int BfmGetIrq( con_Handle_t handle
             , unsigned int *pIrq
             , unsigned int *pFiq )
{
   if ((pIrq==NULL)&&(pFiq==NULL)) {
       return -1;
   }

   int ret=0;
   unsigned int valR;
   ret = BfmGpin( handle, &valR );
   if (ret!=0) return ret;

   if (pIrq!=NULL) *pIrq = (valR>>30)&0x1;
   if (pFiq!=NULL) *pFiq = (valR>>31)&0x1;
